...
{
    load_plugin_textdomain('td_example', false, dirname(__FILE__) . "/languages");
}
add_action("plugins_loaded", "tabledata_load_textdomain");

function td_admin_page(){
    add_menu_page(
...
function td_search_by_name($item){
...
function td_display_table(){
    include_once "dataset.php";
    $orderby = $_REQUEST['orderby'] ?? '';
    $order = $_REQUEST['order'] ?? '';
    if(isset($_REQUEST['s']) && !empty($_REQUEST['s'])){
        $data = array_filter($data, 'td_search_by_name');
    }
    
    if ( isset( $_REQUEST['filter_s'] ) && !empty($_REQUEST['filter_s']) ) {
		$data = array_filter( $data, 'dt_filter_sex' );
	}
...
add_action("admin_menu", "td_admin_page");
